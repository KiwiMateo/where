...
In a typical example, we might have a population dataset and various
subpopulations of interest. If the subpopulations are a partition of the data
(for example, into 5 year age bands), then we can use `group_by` in `dplyr` or
faceting in `ggplot`. In general, however, populations will not be so easy to
apply over, for example if we have some defined by age, others by gender, and
then others as a combination of the two. If a variable allows multiple options 
to be selected (for example, ethnicity in the New Zealand Census), then even
subpopulations defined by a single variable (in this case ethnic groups) cannot
be vectorised with the partitioning functionality (like group by and faceting)
...
than one set of expressions. The `where::do` function hides these manipulations:

```{r}
where::do(with(iris, iris[subgroup, ]),
       subgroup = subgroups)
...
## where::do in a function

We can call `where::do` from within a function to further hide details. For 
example, we could produce subpopulation summaries for the different species of 
iris:  

```{r function_on_parts}
population_summaries <- function(df) where::do(with(df, df[subgroup, ]),
                                            subgroup = subgroups)
...
on_subpopulations <- function(expr,
  ...
  eval(substitute(where::do(expr, subgroup = populations),
...
As some syntactic sugar, there are also two infix versions of `where::do`:

- `%where%` is a full infix version of `where::do` taking the expression as the left
